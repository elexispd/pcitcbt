...
class Course {
	...
	public function get_review($name, $id) {
		...
		// $num = $this->get_course_number();
		$year = date("Y");
		$sql = "SELECT * FROM $table WHERE course_id = ? GROUP BY year, course_no, category ORDER BY year DESC";
	...
	public function get_questions($name, $id) {
		$table = $this->get_table($name);
		$sql = "SELECT * FROM $table WHERE lecturer_id = ? GROUP BY year, course_no, status, category ORDER BY course_no, year DESC";
		$stmt = $this->db->run($sql, [$id]);
		return $stmt->fetchAll();
	}
