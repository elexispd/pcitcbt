...
class Students {
	...
	public function set_ap_number() {
		...
		$sql = "SELECT * FROM students WHERE ap_number = ? AND status = ? AND course_no = ?";
		$stmt = $this->db->run($sql, [$this->ap_number, 1 , $this->course_number]);
		...
		elseif (!preg_match("/^[0-9]*$/", $this->ap_number)) {
			...
			$this->message(0, "Ap/F number already exist For this course");
		}
		// else {
	...
	public function get_student($id) {
		...
		$sql = "SELECT * FROM students WHERE ap_number = ? ORDER BY id DESC";
	...
	public function update_student_profile($fname, $lname, $sg, $state, $gender, $enlist, $command, $promote, $qualification, $apnumber, $year, $no,$ends,$id) {
		...
		if (empty($this->msg)) {
			$id = htmlspecialchars($id);
			$sql = "UPDATE students SET first_name = ?, last_name = ?,  sg = ?,  state_of_origin = ?,  gender = ? , date_of_enlistment = ? , command = ? , date_last_promoted = ? , qualification = ?, ap_number = ?, year_of_birth = ?, course_no = ?, course_ends =?  WHERE id = ?";
	...
	//this method is responsible for displaying exam questions
	public function get_course_questions($name, $id, $c_no, $student_id) {
		$table = $this->get_table($name);
		$year = Date("Y");

		
		$sq = "SELECT * FROM result WHERE student_id = ? AND submitted_at = CURRENT_DATE AND course_id = ?";
		$st = $this->db->run($sq, [$student_id, $id]);
		if ($st->rowCount() > 0) {
			return 1;
		} else {
			$sql = "SELECT * FROM $table WHERE course_id = ? AND year = ? AND status = ? AND course_no = ? AND exam_date = CURRENT_DATE   ORDER BY RAND() LIMIT 20";
			$stmt = $this->db->run($sql, [$id, $year, 1, $c_no]);
			return $stmt->fetchAll();
		}
	}

	//this method will display only practice exams. it is currently not function or linked to the view
	public function get_course_practice_questions($name, $id) {
		$table = $this->get_table($name);
		$sql = "SELECT * FROM $table WHERE course_id = ? ORDER BY RAND() LIMIT 20";
			$stmt = $this->db->run($sql, [$id]);
			return $stmt->fetchAll();
	}
	...
	public function set_result($name, $question_id, $student_id, $c_no, $course_id, $ans, $cat, $duration = null, $time = null) {
		$total = 0;
		$table = $this->get_table($name);
		$year = date("Y");
