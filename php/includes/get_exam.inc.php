...
if (isset($_GET["c_id"]) && isset($_GET['l_id'])) {
    $next_exam_no = $staff_obj->get_question_no($_GET["c_id"]);
    $get_course_q = $staff_obj->get_course_questions($_GET["c_id"], $_GET['l_id']);
    $get_table = $staff_obj->get_table($_GET["c_id"]);

    $get_questions = $course_obj->get_questions($_GET["c_id"],$_GET['l_id']);
...
if (isset($_GET['c_id']) && isset($_GET['id'])) {
    ...
    // $get_duration = $student_obj->get_exam_duration($_GET['c_id'], $_GET['id']);
}
...
if (isset($_SESSION["lecturer"])) {
    $course = $course_obj->get_course_lecturer_two($_SESSION['lecturer']);
    $course_t = "";
    foreach ($course as  $value) {
        $course_t = $value["course_title"];
    }
    $update_exam_status = $staff_obj->update_exam_status(strtolower($course_t)."_exam");
}

//display all questions to the lecture for editting
if (isset($_GET['c_id']) && isset($_GET['l_id']) && isset($_GET['year']) && isset($_GET['co_no']) ) {
    $get_s_questions = $course_obj->get_set_questions($_GET['c_id'], $_GET['l_id'], $_GET['co_no'], $_GET['year'], $_GET['st'], $_GET['cat']);
}